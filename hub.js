(() => {
 ...
 function render() {
  ...
  document.getElementById('trips').innerHTML=trips.map(t=>`<a class="trip" href="${safe(t.path)}">${t.image?`<img src="${safe(t.image)}" alt="${e(t.title)}旅行海报" loading="lazy">`:'<div class="noimage">◈</div>'}<div><div class="meta"><span class="badge">${status(t)}</span><span>${e(t.type || '旅行攻略')}</span></div><h3>${e(t.title)}</h3><p class="dates">${e(t.dates)} · ${e(t.people)}</p><p>${e(t.summary)}</p><span class="open">查看旅程 ↗</span></div></a>`).join('') || '<p>尚未收录旅途记录。</p>';
 }
