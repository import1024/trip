(() => {
 ...
 function render() {
  ...
  document.getElementById('total').textContent=`${trips.length} 份计划`;
  document.getElementById('trips').innerHTML=trips.map(t=>`<a class="trip" href="${safe(t.path)}">${t.image?`<img src="${safe(t.image)}" alt="${e(t.title)}旅行海报" loading="lazy">`:'<div class="noimage">◈</div>'}<div><div class="meta"><span class="badge">${status(t)}</span><span>${e(t.type || '旅行攻略')}</span></div><h3>${e(t.title)}</h3><p class="dates">${e(t.dates)} · ${e(t.people)}</p><p>${e(t.summary)}</p><span class="open">打开计划 ↗</span></div></a>`).join('') || '<p>暂未添加旅行计划。</p>';
 }
